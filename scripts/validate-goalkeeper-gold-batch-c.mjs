...
const all = [...GOALKEEPER_GOLD_PILOT_10, ...GOALKEEPER_GOLD_BATCH_A_15, ...GOALKEEPER_GOLD_BATCH_B1_10, ...GOALKEEPER_GOLD_BATCH_C_15];
...
if (GOALKEEPER_GOLD_BATCH_C_15.length !== 15 || all.length !== 50) errors.push('Source count mismatch');
...
const report = { status: errors.length ? 'FAIL' : 'GOALKEEPER GOLD BATCH C READY FOR HUMAN COACH REVIEW', generatedAt: new Date().toISOString(), sourceGoldCount: all.length, fullTarget: GOALKEEPER_GOLD_FAMILIES.length, batchCCount: GOALKEEPER_GOLD_BATCH_C_15.length, sourceHash: hash(all), coverage, semanticRisks, lockCheck, pivotCheck, goalkeeperCheck, errors, warnings };
writeFileSync(join(root, 'scripts/goalkeeper-gold-batch-c-validation.json'), `${JSON.stringify(report, null, 2)}\n`);
console.log(JSON.stringify({ status: report.status, sourceGoldCount: report.sourceGoldCount, fullTarget: report.fullTarget, batchCCount: report.batchCCount, semanticRisks: semanticRisks.length, lockedGoldStable: Object.values(lockCheck).every(({ ok }) => ok), pivotGoldStable: pivotCheck.ok, goalkeeperBankStable: goalkeeperCheck.ok, errors: errors.length, warnings: warnings.length }, null, 2));
