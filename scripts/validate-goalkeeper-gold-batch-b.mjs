...
const all = [...GOALKEEPER_GOLD_PILOT_10, ...GOALKEEPER_GOLD_BATCH_A_15, ...GOALKEEPER_GOLD_BATCH_B1_10];
...
if (GOALKEEPER_GOLD_BATCH_B1_10.length !== 10 || all.length !== 35) errors.push('Source count mismatch');
...
const report = { status: errors.length ? 'FAIL' : 'GOALKEEPER GOLD BATCH B READY FOR HUMAN COACH REVIEW', generatedAt: new Date().toISOString(), sourceGoldCount: all.length, fullTarget: GOALKEEPER_GOLD_FAMILIES.length, batchBCount: GOALKEEPER_GOLD_BATCH_B1_10.length, sourceHash: hash(all), coverage, semanticRisks, lockCheck, pivotCheck, goalkeeperCheck, errors, warnings };
writeFileSync(join(root, 'scripts/goalkeeper-gold-batch-b-validation.json'), `${JSON.stringify(report, null, 2)}\n`);
console.log(JSON.stringify({ status: report.status, sourceGoldCount: report.sourceGoldCount, fullTarget: report.fullTarget, batchBCount: report.batchBCount, semanticRisks: semanticRisks.length, lockedGoldStable: Object.values(lockCheck).every(({ ok }) => ok), pivotGoldStable: pivotCheck.ok, goalkeeperBankStable: goalkeeperCheck.ok, errors: errors.length, warnings: warnings.length }, null, 2));
