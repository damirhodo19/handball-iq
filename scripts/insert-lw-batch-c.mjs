#!/usr/bin/env node
/**
 * Append LW Batch C (scn_bank_975–981) only.
 * Does not touch 941–974, legacy LW, or LB/RB/CB/RW.
 */
import { readFileSync, writeFileSync } from 'fs';
import { join, dirname } from 'path';
import { fileURLToPath } from 'url';
import crypto from 'crypto';

const root = join(dirname(fileURLToPath(import.meta.url)), '..');
const bankPath = join(root, 'content/scenario-bank/scenarios.json');
const batchPath = join(root, 'scripts/scenario-bank/data/lw-parts/lw-families-c.json');
const lockPath = join(root, 'scripts/.lw-batch-c-lock-before.json');

const bank = JSON.parse(readFileSync(bankPath, 'utf8'));
const batch = JSON.parse(readFileSync(batchPath, 'utf8'));
const lock = JSON.parse(readFileSync(lockPath, 'utf8'));

const hash = (v) => crypto.createHash('sha256').update(JSON.stringify(v)).digest('hex');
const hashPos = (pos) => hash(bank.filter((s) => s.primaryPosition === pos));

for (const [k, pos] of Object.entries({
  LB: 'Left Back',
  RB: 'Right Back',
  CB: 'Centre Back',
  RW: 'Right Wing',
})) {
  if (hashPos(pos) !== lock[k]) throw new Error(`LOCK HASH CHANGED BEFORE INSERT: ${k}`);
}
for (const [id, h] of Object.entries(lock.seeds941to974)) {
  const cur = hash(bank.find((s) => s.id === id));
  if (cur !== h) throw new Error(`Locked LW seed changed before insert: ${id}`);
}

if (batch.length < 6 || batch.length > 12) throw new Error(`Unexpected batch size ${batch.length}`);
if (bank.some((s) => /^scn_bank_97[5-9]$|^scn_bank_98[0-6]$/.test(s.id))) {
  throw new Error('Batch C IDs already present — abort');
}
if (bank.some((s) => s.id === 'scn_bank_987')) throw new Error('987 must not exist');

const added = [];
for (const f of batch) {
  const n = Number(String(f.id).replace('scn_bank_', ''));
  if (n < 975 || n > 986) throw new Error(`ID out of range: ${f.id}`);
  const qs = f.answers.map((a) => a.quality).join(',');
  if (qs !== 'optimal,good,risky,poor') throw new Error(`${f.id} qualities=${qs}`);

  const skillTags = [
    ...(f.skillTags || []),
    ...(f.perception && !(f.skillTags || []).includes('perception') ? ['perception'] : []),
    ...(f.numerical && f.numerical !== '6v6' && !(f.skillTags || []).some((t) => String(t).startsWith('numerical:'))
      ? [`numerical:${f.numerical}`]
      : []),
    ...(f.gameState && f.gameState !== 'none' ? [`gameState:${f.gameState}`] : []),
    `family:${f.familyKey}`,
    `batch:${f.batchId}`,
  ];

  bank.push({
    id: f.id,
    title: f.title,
    category: 'Left Wing',
    primaryPosition: 'Left Wing',
    secondaryPositions: [],
    difficulty: f.difficulty,
    pressureLevel: f.pressureLevel,
    attackOrDefence: f.attackOrDefence,
    matchPhase: f.matchPhase,
    minute: f.minute,
    score: f.score,
    defensiveSystem: f.defensiveSystem,
    situation: f.situation,
    question: f.question,
    answers: f.answers,
    explanation: f.explanation,
    whyCorrectOverSecondBest: f.whyCorrectOverSecondBest,
    skillTags,
    qualityScore: f.qualityScore ?? 8.6,
  });
  added.push(f.id);
}

for (const [k, pos] of Object.entries({
  LB: 'Left Back',
  RB: 'Right Back',
  CB: 'Centre Back',
  RW: 'Right Wing',
})) {
  if (hashPos(pos) !== lock[k]) throw new Error(`LOCK BROKEN AFTER INSERT: ${k}`);
}
for (const [id, h] of Object.entries(lock.seeds941to974)) {
  if (hash(bank.find((s) => s.id === id)) !== h) throw new Error(`941-974 mutated: ${id}`);
}

writeFileSync(bankPath, JSON.stringify(bank, null, 2) + '\n');
writeFileSync(
  join(root, 'scripts/lw-gold-batch-c-insert.json'),
  JSON.stringify(
    {
      inserted: added,
      count: added.length,
      temporaryLwTotal: bank.filter((s) => s.primaryPosition === 'Left Wing').length,
      locksOk: true,
      locked941to974Ok: true,
    },
    null,
    2,
  ) + '\n',
);

console.log(JSON.stringify({ inserted: added, lw: bank.filter((s) => s.primaryPosition === 'Left Wing').length }, null, 2));
